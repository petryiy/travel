...
import type { CollaboratorRole, SavedTripSummary } from '@/types/travel'
...
interface Props {
  ...
  onLeaveTrip: (tripId: string) => Promise<boolean>
}
...
const ROLE_BADGES: Record<Exclude<CollaboratorRole, 'owner'>, { label: string; cls: string }> = {
  editor: { label: 'Can edit', cls: 'bg-[#e3efe2] text-[#496248]' },
  commenter: { label: 'Can comment', cls: 'bg-[#e3eef2] text-[#3f6070]' },
  viewer: { label: 'View only', cls: 'bg-[#efe7d8] text-[#7d6c58]' },
}

function isShared(trip: SavedTripSummary) {
  return Boolean(trip.role && trip.role !== 'owner')
}
...
export function Dashboard({ savedTrips, isLoadingTrips, onNewTrip, onOpenTrip, onDeleteTrip, onLeaveTrip }: Props) {
  const [confirmId, setConfirmId] = useState<string | null>(null)
  const [actingId, setActingId] = useState<string | null>(null)

  const ownedTrips = savedTrips.filter((trip) => !isShared(trip))
  const sharedTrips = savedTrips.filter(isShared)

  async function handleConfirm(trip: SavedTripSummary) {
    setActingId(trip.id)
    if (isShared(trip)) {
  ...
  function renderCard(trip: SavedTripSummary) {
    ...
    const roleBadge = shared && trip.role && trip.role !== 'owner' ? ROLE_BADGES[trip.role] : null
      ...
      >
        ...
        >
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              {shared && (
                <p className="mt-1 truncate text-xs font-semibold text-[#8a7a64]">
                  Shared by {trip.ownerName || 'a friend'}
                </p>
              )}
            </div>
            <div className="flex shrink-0 flex-col items-end gap-1.5">
              ...
              {roleBadge && (
                <span className={`rounded-full px-3 py-1 text-xs font-bold ${roleBadge.cls}`}>
                  {roleBadge.label}
                </span>
              )}
              {trip.isPublished && (
        ...
        {isConfirming ? (
          <div className="absolute bottom-3 right-3 flex items-center gap-1.5 rounded-full border border-[#e6c7bd] bg-white/95 px-2 py-1 shadow-sm backdrop-blur sm:bottom-4 sm:right-4">
            <span className="px-1 text-xs font-semibold text-[#8a5438]">{shared ? 'Leave?' : 'Delete?'}</span>
            <button
              type="button"
              onClick={() => void handleConfirm(trip)}
              disabled={isActing}
              className="rounded-full bg-[#b4493a] px-2.5 py-1 text-xs font-bold text-white transition hover:bg-[#9c3f31] disabled:opacity-60"
            >
              {isActing ? (shared ? 'Leaving...' : 'Deleting...') : 'Yes'}
            </button>
            <button
              type="button"
          ...
          <button
            ...
            className="absolute bottom-3 right-3 rounded-full border border-transparent px-3 py-1.5 text-xs font-bold text-[#a0927f] opacity-0 transition hover:border-[#e6c7bd] hover:bg-white hover:text-[#b4493a] focus:opacity-100 focus-visible:opacity-100 group-hover:opacity-100 sm:bottom-4 sm:right-4"
          >
            Leave
          </button>
        ) : (
        ...
        <div className="mx-auto flex w-full max-w-7xl flex-col gap-3 px-4 py-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between sm:gap-4 sm:px-6 sm:py-4 lg:px-8">
          <div>
            ...
            <p className="mt-1 text-sm text-[#7d6c58]">
              {isLoadingTrips
                ? 'Loading saved plans...'
                : `${ownedTrips.length} saved ${ownedTrips.length === 1 ? 'plan' : 'plans'}${
                    sharedTrips.length > 0 ? ` · ${sharedTrips.length} shared with you` : ''
                  }`}
            </p>
          </div>
      ...
      <section className="mx-auto w-full max-w-7xl px-4 pt-4 pb-[calc(2.5rem+env(safe-area-inset-bottom))] sm:px-6 sm:pt-8 sm:pb-16 lg:px-8">
        ...
        {ownedTrips.length > 0 && (
          <div className="grid gap-3 sm:grid-cols-2 sm:gap-4 xl:grid-cols-3">
            {ownedTrips.map(renderCard)}
          </div>
        )}

        {sharedTrips.length > 0 && (
          <>
            <div className="mb-4 mt-8 flex items-center gap-3 sm:mb-5">
              <h2 className="text-sm font-bold uppercase tracking-[0.14em] text-[#7d6c58]">Shared with you</h2>
              <span className="rounded-full bg-[#e3efe2] px-2.5 py-0.5 text-xs font-bold text-[#496248]">
                {sharedTrips.length}
              </span>
            </div>
            <div className="grid gap-3 sm:grid-cols-2 sm:gap-4 xl:grid-cols-3">
              {sharedTrips.map(renderCard)}
            </div>
          </>
        )}
      </section>
